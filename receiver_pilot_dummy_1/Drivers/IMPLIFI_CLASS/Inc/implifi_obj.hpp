...
class ImpLiFiClass
{
	public:
		...
		uint8_t encoder_manchester[2*(RS_MESSAGE_LENGTH + ECC_LENGTH)];
...
bool ImpLiFiClass::DecryptData(uint8_t manchester_in[]) // Returns 0 if successful
{
	...
	//error_code = rs.Decode(rs_encoded_buff, reed_solomon_repaired, &err_pos, num_err, num_errs_val);
	if(rs.Decode(rs_encoded_buff, reed_solomon_repaired, NULL, 0, num_errs_val) != 0)
	{
		return false;
	}
	rs.Decode(rs_encoded_buff, reed_solomon_repaired, NULL, 0, num_errs_val);
